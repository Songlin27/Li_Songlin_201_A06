...
[https://github.com/Songlin27/Li_Songlin_201_A06](https://github.com/Songlin27/Li_Songlin_201_A06)
...
rounding = function(x) {
  if (x < 100) {
    return(round(x, -1))  #nearest tens, hundredth, thousands depending on condition
  } else if (x < 1000) {
    return(round(x, -2))  
  } else {
    return(round(x, -3))  
...
apply(mat_rand, MARGIN = c(1, 2), FUN = rounding)
...
average_temps = colMeans(max_temps[, 3:14])
print(average_temps)
...
# slice to include only Colorado 
...
median = apply(max_temps_CO[, 3:14], 1, median)

# find year of highest and lowest median
max = max(median)
min = min(median)


max_year = max_temps_CO$year[which.max(median)]
min_year = max_temps_CO$year[which.min(median)]

# Print the results
paste0("The maximum median annual temperature of ", max, " occurred in ", max_year,
    " and the minimum median annual temperature of ", min, " occurred in ", min_year)
...
sums = function(x) {
  if (x<10) {
    return (x)
  } else {     #store last digit then redo without lqst digit
    last_digit = x%%10
    remainder = (x-last_digit) / 10 
    return(last_digit + sums(remainder))
  }
}
